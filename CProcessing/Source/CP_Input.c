...
void CP_InputInit(void)
{
	...
	CP_MouseUpdate();
	CP_MouseUpdate(); // intentionally called twice to setup curr and prev mouse values
}
...
int CP_GamepadConvertButtonToXInput(CP_GAMEPAD button)
{
...
int isValidGamepadIndex(int index)
{
...
{
	if (CP_IsValidGamepad(button) && isValidGamepadIndex(gamepadIndex))
	{
		// Wasn't pressed last frame and is pressed this frame
		int convertedButton = CP_GamepadConvertButtonToXInput(button);
...
{
	return isValidGamepadIndex(gamepadIndex) && gamepad_connected[gamepadIndex];
}
...
//------------------------------------------------------------------------------
// Extended Functions for grading purposes:
//------------------------------------------------------------------------------
CP_API void CP_Input_ForceMouseTriggered(CP_MOUSE button)
{
	mouse_states_previous[button] = false;
	mouse_states_current[button] = true;
}
...
CP_API void	CP_InputForceKeyTriggered(CP_KEY keyCode) {
	key_states_previous[keyCode] = false;
	key_states_current[keyCode] = true;
	key_any_triggered = true;
	key_any_down = true;
}